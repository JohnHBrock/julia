...
static jl_typename_t* jl_type_extract_name(jl_value_t *t1)
{
    if (jl_is_typector(t1))
        t1 = (jl_value_t*)((jl_typector_t*)t1)->body;
    ...
    if (jl_typeof(ml_or_cache.unknown) == (jl_value_t*)jl_typemap_level_type) {
        ...
        if (ty) {
            if (cache->tname != (void*)jl_nothing && jl_is_type_type(ty)) {
                jl_typename_t *a0 = jl_type_extract_name(jl_tparam0(ty));
                if (a0) {
            ...
            if (cache->name1 != (void*)jl_nothing) {
                jl_typename_t *a0 = jl_type_extract_name(ty);
                if (a0) {
...
jl_typemap_entry_t *jl_typemap_assoc_exact(union jl_typemap_t ml_or_cache, jl_value_t **args, size_t n, int8_t offs)
{
    ...
    if (jl_typeof(ml_or_cache.unknown) == (jl_value_t*)jl_typemap_level_type) {
        ...
        jl_typemap_entry_t *ml = jl_typemap_node_assoc_exact(cache->linear_leaf, args, n);
        if (ml) return ml;
        if (ty) {
            if (ty == (jl_value_t*)jl_datatype_type && cache->tname != (void*)jl_nothing) {
                ml_or_cache = mtcache_hash_lookup(cache->tname, ((jl_datatype_t*)a1)->name, 3, offs);
                jl_typemap_entry_t *ml = jl_typemap_assoc_exact(ml_or_cache, args, n, offs+1);
                if (ml) return ml;
            }
            if (cache->name1 != (void*)jl_nothing) {
                ml_or_cache = mtcache_hash_lookup(cache->name1, ((jl_datatype_t*)ty)->name, 2, offs);
                jl_typemap_entry_t *ml = jl_typemap_assoc_exact(ml_or_cache, args, n, offs+1);
                if (ml) return ml;
            }
        }
        ml = jl_typemap_node_assoc_exact(cache->linear, args, n);
        if (ml) return ml;
